<template>
    <div>
        <li @click="generatePDF"><a><i class="fa-solid fa-file-pdf"></i> Print Invoice</a></li>
    </div>
</template>
    ...
    if (parseInt(satang) === 0) {
        ...
        if (satang < 10) {
            ...
            if (satang[1] === '0') {
                ...
                bahtText += ' ' + number[parseInt(satang[0])] + ' ten ' + number[parseInt(satang[1])] + ' satang';
            }
        }
    }
...
const generatePDF = () => {
    ...
    image.onload = () => {
        ...
        // Customer Info
        doc.text(invoice[`customer_name${lang === 'ENG' ? '_eng' : ''}`], 85, 107);
        doc.text(invoice[`customer_address${lang === 'ENG' ? '_eng' : ''}`], 60, 133, { maxWidth: 350, lineHeightFactor: 0.75 });
        ...
        doc.text(invoice.total_discount ? "-" + formatNumberValue(invoice.total_discount) : "-", 520, 527, { align: 'right' });
        doc.text(invoice.vat_amount ? formatNumberValue(invoice.vat_amount) : "-", 520, 540, { align: 'right' });
        doc.setFontSize(18);
        doc.text(invoice.grand_total ? formatNumberValue(invoice.grand_total) : "-", 520, 560, { align: 'right' });
        doc.setFontSize(14);
        doc.text(invoice.total_with_holding_tax ? formatNumberValue(invoice.total_with_holding_tax) : "-", 520, 575, { align: 'right' });
        doc.text(invoice.payment_total ? formatNumberValue(invoice.payment_total) : "-", 520, 588, { align: 'right' });
