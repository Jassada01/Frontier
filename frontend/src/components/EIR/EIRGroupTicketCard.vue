      ...
      <div class="p-6 bg-white flex-grow">
        ...
        <div class="space-y-3" v-if="eir.invoices?.length">
          <div 
            ...
            class="p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer space-y-3"
          ...
          >
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
              <div>
                <p class="text-gray-500">หัก ณ ที่จ่าย</p>
                <p class="font-medium text-gray-700 mt-0.5">{{ formatAmount(invoice.total_with_holding_tax || 0) }}</p>
              </div>
              
              <div>
                <p class="text-800-500">ยอดที่ต้องชำระ</p>
                <p class="font-medium text-blue-800 mt-0.5">{{ formatAmount(invoice.payment_total) }}</p>
              </div>
            </div>

            <!-- Dashed line divider - แสดงเฉพาะระหว่าง invoice ไม่แสดงหลังตัวสุดท้าย -->
    <div 
      v-if="index !== eir.invoices.length - 1" 
