...
const handleContainerNumberInput = (index) => {
    formData.value.containers[index].containerNumber = formData.value.containers[index].containerNumber.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
};
...
const handleSubmit = async () => {
    ...
    try {
        const response = await axios.post(`${CONFIG.API_SERVER}/api/container-returns/add`, formData.value);
...
<template>
    <div class="container-return-form">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div class="flex items-center">
                <label class="w-1/4">ใบคืนตู้: <span class="text-red-500">*</span></label>
                <div class="w-3/4">
                    <input type="file" id="returnDocumentInput" @change="handleReturnDocumentUpload"
                    ...
                    <label for="returnDocumentInput" class="btn btn-outline cursor-pointer">
                        ...
                        <span v-else><span class="loading loading-spinner loading-sm"></span>กำลังอัปโหลด...</span>
                    </label>
                    <div v-if="formData.returnDocumentUrls.length > 0" class="mt-2 flex flex-wrap gap-2">
                        <div v-for="(url, index) in formData.returnDocumentUrls" :key="index" class="relative">
                            <div v-if="isImageFile(url)" class="w-24 h-24 rounded overflow-hidden">
                                <img :src="url" :alt="`Return Document ${index + 1}`"
            ...
            <div v-for="(container, index) in formData.containers" :key="index"
                ...
                <div class="flex items-center mb-2">
                    <label :for="'containerNumber' + index" class="w-1/4">เบอร์ตู้: <span
                            class="text-red-500">*</span></label>
                    <input :id="'containerNumber' + index" v-model="container.containerNumber"
                        @input="handleContainerNumberInput(index)" type="text" required
                        class="input input-bordered w-3/4" />
                </div>
                <div class="flex items-center mb-2">
                    <label :for="'containerSize' + index" class="w-1/4">ขนาดตู้:</label>
                    <select :id="'containerSize' + index" v-model="container.containerSize" required
                ...
                <div class="flex items-center mb-2">
                    <label class="w-1/4">รูปตู้: <span class="text-red-500">*</span></label>
                    <div class="w-3/4">
                        <input type="file" :id="'containerImageInput' + index"
                            @change="(e) => handleContainerImageUpload(e, index)" accept="image/*" multiple
                            class="hidden" />
                        ...
                        <div v-if="container.imageUrls.length > 0" class="mt-2 flex flex-wrap gap-2">
                            <div v-for="(url, imgIndex) in container.imageUrls" :key="imgIndex" class="relative">
                                ...
                                <button type="button" @click="() => handleRemoveFile('image', imgIndex, index)"
                ...
                <button type="button" @click="removeContainer(index)" class="btn btn-error btn-sm mt-2"
