<template>
  <div class="min-h-screen flex flex-col items-center justify-center p-2">
    <!-- แสดง TraditionalLogin component เมื่อไม่ได้อยู่ใน LINE -->
    <template v-if="!liff.isInClient()">
      <TraditionalLogin />
    </template>
    <div  v-if="liff.isInClient()" class="card lg:card-side bg-base-100 shadow-xl max-w-4xl w-full">
        ...
        <div v-if="showRegisterForm && userType === 'Client'" class="form-control mt-6">
          ...
          <form @submit.prevent="registerUser">
            ...
            <div class="form-control">
              <label class="label">
                <span class="label-text"
                  >ชื่อในระบบ<span class="text-error font-bold"> *</span></span
                >
              </label>
            ...
            <div class="form-control">
              <label class="label">
                <span class="label-text"
                  >ชื่อบริษัท<span class="text-error font-bold"> *</span></span
                >
              </label>
...
// Instead, render the traditional login component
import TraditionalLogin from './TraditionalLogin.vue'
...
const checkUserInDatabase = async (userId) => {
  try {
    const response = await axios.get(
      `${CONFIG.API_SERVER}/api/line_user_profiles/get?line_user_id=${userId}`
    )
...
const registerUser = async () => {
  ...
  try {
    await axios.post(`${CONFIG.API_SERVER}/api/line_user_profiles/add`, {
      ...
      picture_url: userProfile.value.pictureUrl || defaultProfilePicture, // Use default if pictureUrl is empty
    ...
    // Show success message and reload page after user clicks OK
    Swal.fire({
      icon: 'success',
...
const handleDriverFormSubmitted = async (driverId, companyDetails) => {
  try {
    ...
    Swal.fire(
      'Error',
      'เกิดข้อผิดพลาดในการลงทะเบียนผู้ใช้ในระบบ Line: ' +
  ...
  if (liff.isInClient()) {
    ...
    /*
        userProfile.value = {
            ...
            statusMessage: "",
            userId: "Ue2ff72aa5df21f6c0047be7f883d57ffL"
        }
